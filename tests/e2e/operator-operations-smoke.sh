...
wait_task_succeeded() {
	...
	for _ in $(seq 1 90); do
		phase="$(kubectl -n "${NAMESPACE}" get taskruns.kember.openflood.org echo -o jsonpath='{.status.phase}' 2>/dev/null || true)"
		if [[ "${phase}" == "Succeeded" ]]; then
			return 0
		fi
		if [[ "${phase}" == "Failed" || "${phase}" == "TimedOut" || "${phase}" == "Rejected" || "${phase}" == "Cancelled" ]]; then
			kubectl -n "${NAMESPACE}" get taskruns.kember.openflood.org echo -o yaml >&2
			return 1
		fi
		sleep 1
	done
	kubectl -n "${NAMESPACE}" get taskruns.kember.openflood.org echo -o yaml >&2 || true
	return 1
}
...
wait_task_succeeded
job_name="$(kubectl -n "${NAMESPACE}" get taskruns.kember.openflood.org echo -o jsonpath='{.status.jobRef.name}')"
[[ -n "${job_name}" ]]
job_count="$(kubectl -n "${NAMESPACE}" get jobs -l kember.openflood.org/taskrun-uid -o name | wc -l | tr -d ' ')"
[[ "${job_count}" == "1" ]]
...
[[ "$(kubectl -n "${NAMESPACE}" get taskruns.kember.openflood.org echo -o jsonpath='{.status.phase}')" == "Succeeded" ]]
[[ "$(kubectl -n "${NAMESPACE}" get jobs -l kember.openflood.org/taskrun-uid -o name | wc -l | tr -d ' ')" == "1" ]]
[[ "$(kubectl -n "${NAMESPACE}" get taskruns.kember.openflood.org echo -o jsonpath='{.status.jobRef.name}')" == "${job_name}" ]]
